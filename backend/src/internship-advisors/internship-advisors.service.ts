...
export class InternshipAdvisorsService {
  ...
  findAll(order?, skip?: number, take?: number, filter?: BaseFilter, campusId?: number): Promise<[InternshipAdvisor[], number]> {
    const { q } = filter;
    const whereClause = q ? { name: Raw(alias => `${alias} ILIKE '%${q}%'`) } : null;
    return this.internshipAdvisorRepository.findAndCount({ order, skip, take, where: {  ...whereClause, campus: campusId }, relations: ['user'] });
  }

  create(internshipAdvisor: CreateInternshipAdvisorDTO, campusId?: number): Promise<InternshipAdvisor> {
    internshipAdvisor.campus = campusId ? campusId : internshipAdvisor.campus;
    const internshipAdvisorObj = this.internshipAdvisorRepository.create({ ...internshipAdvisor });
